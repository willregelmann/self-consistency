...
\textbf{(Rigorous for real-subspace invariance; Sketch for uniqueness.)}
\label{prop:riem_classical}
Let $\gamma \in \mathbb{R}$ (i.e., $\theta = 0$) and $h \in
\mathbb{R}^N$. Then:
\begin{enumerate}
\item[(a)] \textbf{(Rigorous.)} The real subspace $\mathbb{R}^N$ is invariant
under the self-consistency map $F$, and every conditional density matrix
of a real fixed point has real entries: $\mathrm{Im}(\rho_{S|c}) = 0$
for all~$c$.
\item[(b)] \textbf{(Sketch.)} The fixed point $\psi^*$ is unique in
$(\mathbb{C}^N, \|\cdot\|_2)$, hence real-valued.
\end{enumerate}
\end{proposition}
...
The real subspace is therefore invariant under $F$. Any fixed point in
$\mathbb{R}^N$ then gives real conditional slices
$\mu_{S|c}(s) = \psi^*_{(c,s)} \in \mathbb{R}$,
so $\rho_{S|c} = |\mu_{S|c}\rangle\langle\mu_{S|c}| /
\langle\mu_{S|c}|\mu_{S|c}\rangle$ is a real outer product divided by a
real scalar.

\emph{Part~(b) (Sketch).} Uniqueness of the fixed point in $\mathbb{C}^N$
would imply that it lies in $\mathbb{R}^N$ (since $\mathbb{R}^N$ is
$F$-invariant and hence contains at least one fixed point by Brouwer).
The companion paper~\cite{fixed_point} establishes a Banach contraction for
