...
companion paper~\cite{fixed_point} argues, at Sketch level, that the perturbative
...
an $O(1)$ fraction of its norm in the imaginary part (after global
phase alignment): $\sim\!25$--$40\%$ for $\theta = 1$ and
$h \sim \mathrm{Uniform}[0.5, 1.5]$, growing with the system size $N$
toward an analytically computable limit and depending on the alignment
convention, but \emph{not} on subsystem rank
(Section~\ref{sec:toy_model}). However, when a
...
\begin{remark}[Machine-checked formalization]
\label{rem:lean_entropy_excess}
The two inequalities of Lemma~\ref{lem:entropy_excess} are formalized in
Lean~4 against Mathlib (\texttt{programs/co-emergence/lean/}; milestone
CE-5, issue \#45). Part~(a)---purity decrease for all ranks, via Fact~3
and the entrywise modulus bound---is proved in full as
\texttt{purity\_decrease}, with \texttt{trace\_rho\_sq} certifying that
the formalized quantity $\sum_{jk}|\rho_{jk}|^2$ is indeed
$\mathrm{Tr}(\rho^2)$ (using the Hermiticity of $\rho$); the bridge from
the Lean statement to the paper's matrix claim is fully formalized.
Part~(b) is machine-checked at the level of the paper's key scalar step:
\texttt{rank2\_entropy\_excess} proves that $p_\theta \le p_0$ on
$[\tfrac{1}{2},1]$ implies $\mathrm{Srank2}(p_\theta) \ge
\mathrm{Srank2}(p_0)$ (binary-entropy monotonicity, ``more uniform
$\Rightarrow$ higher entropy''). Three structural links between this
scalar lemma and the paper's matrix claim are supplied as hypotheses
and not re-derived in Lean: (i)~the eigenvalue identification (the
rank-2 $\rho(\theta)$'s singular values satisfy $\sigma_1^2+\sigma_2^2=1$,
so the spectrum is $\{p,1-p\}$ with $p=\sigma_1^2$); (ii)~the entropy
connection $S(\rho)=\mathrm{Srank2}(\sigma_1^2)$; and (iii)~Fact~2
($\sigma_1(\theta)\le\sigma_1$, proved analytically above). A
\texttt{\#print axioms} audit shows the theorems rest only on Mathlib's
standard axioms (\texttt{propext}, \texttt{Classical.choice},
\texttt{Quot.sound}), with no \texttt{sorry}. Consistent with
Remark~\ref{rem:vn_general_rank}, the general-rank entropy excess is
\emph{not} formalized: it is false. Only the corrected inequalities
(cf.\ the separability equality condition above) are encoded, never the
retracted ``all entries equal'' characterization.
\end{remark}
...
fixed-point wavefunction $\psi^*$ directly. Its imaginary fraction
admits an \emph{exact reduction} \emph{(Rigorous)}: since
$\psi^*_\sigma \propto e^{(-1+i\theta)R_\sigma}$, the magnitude
$|\psi^*_\sigma|^2 = p_\sigma$ is phase-independent and the phase is
$-\tfrac{\theta}{2}\log p_\sigma$ up to a global constant, so after
aligning a reference component $r$ to be real,
\begin{equation}
\label{eq:im_frac_reduction}
\mathrm{im\_frac}^2
= \sum_\sigma p_\sigma \,
  \sin^2\!\Big(\tfrac{\theta}{2}\,\log\tfrac{p_r}{p_\sigma}\Big).
\end{equation}
The imaginary fraction is therefore a deterministic functional of the
Born magnitude spectrum $\{p_\sigma\}$ and $\theta$ alone; it vanishes
identically for the Riemannian ($\theta = 0$) fixed point. Equation
\eqref{eq:im_frac_reduction} reproduces the directly computed
imaginary fraction to machine precision ($\sim\!10^{-16}$) and shows
that the value is controlled by $\theta$, the spread of $h$, the total
dimension $N$, and the alignment convention---\textbf{not} by subsystem
rank. At fixed $N$ it is independent of the bipartition rank
$\min(d_1,d_2)$ (the earlier ``rank dependence'' was a confound of $N$
with rank), and the previously reported $\sim\!25\%$ and $\sim\!34\%$
figures are the same fixed points under the optimal
(minimal-imaginary-norm) and max-magnitude alignment conventions
respectively. For $\theta = 1$, $h \sim \mathrm{Uniform}[0.5,1.5]$, and
the max-magnitude alignment, $\mathrm{im\_frac}$ rises with $N$ as
...
self-consistency map produces complex fixed points with $\sim 25$--$40\%$
imaginary content (set by $\theta$, the spread of $h$, $N$, and the
alignment convention, with an exact reduction to the magnitude
spectrum; see eq.~\eqref{eq:im_frac_reduction}), at all sizes tested
through $N = 256$; the marginals satisfy
...
\mathbb{R}^N$. Then:
\begin{enumerate}
\item[(a)] \textbf{(Rigorous.)} The self-consistency map $F$ sends all of
$\mathbb{C}^N$ into $\mathbb{R}^N_{>0}$. Consequently \emph{every} fixed
point of $F$ --- regardless of uniqueness --- is real-valued, and every
conditional density matrix of a fixed point has real entries:
$\mathrm{Im}(\rho_{S|c}) = 0$ for all~$c$.
\item[(b)] \textbf{(Sketch.)} The fixed point $\psi^*$ is unique in
$(\mathbb{C}^N, \|\cdot\|_2)$.
\end{enumerate}
\end{proposition}

\begin{proof}
\emph{Part~(a).} The curvature $R_\sigma(\psi) = h_\sigma + \sum_j \alpha_j M_j(\sigma_j) +
\beta\,|\psi_\sigma|^2$ depends on $\psi$ only through $|\psi_\sigma|^2 \in
\mathbb{R}_{\ge 0}$, so $R_\sigma(\psi) \in \mathbb{R}$ for \emph{every}
$\psi \in \mathbb{C}^N$, not merely for real $\psi$. With $\gamma \in
\mathbb{R}$, this gives $e^{\gamma R_\sigma(\psi)} \in \mathbb{R}_{>0}$ for
every $\psi \in \mathbb{C}^N$, so the self-consistency map
$F(\psi)_\sigma = e^{\gamma R_\sigma(\psi)} / \|e^{\gamma R(\psi)}\|_2$
sends all of $\mathbb{C}^N$ into $\mathbb{R}^N_{>0}$ --- the real
subspace is not merely invariant, it is the image of $F$ on its entire
domain. Hence any fixed point $\psi^* = F(\psi^*)$ automatically lies in
$\mathbb{R}^N_{>0}$, whether or not $\psi^*$ is unique. This gives real
conditional slices $\mu_{S|c}(s) = \psi^*_{(c,s)} \in \mathbb{R}$, so
$\rho_{S|c} = |\mu_{S|c}\rangle\langle\mu_{S|c}| /
\langle\mu_{S|c}|\mu_{S|c}\rangle$ is a real outer product divided by a
real scalar.

\emph{Part~(b) (Sketch).} Existence of a fixed point of $F$ is not in
question here (Part~(a) shows any such fixed point is automatically real);
what remains open is \emph{uniqueness} in $\mathbb{C}^N$. The companion
paper~\cite{fixed_point} establishes a Banach contraction for
the \emph{field-theoretic} semiclassical Einstein equation (Sketch,
permanently at this generality; Section~\ref{sec:level2}), but this
argument does not directly apply to the finite-dimensional map $F$ on
$\mathbb{C}^N$ used here. An independent contraction argument for $F$
on $(\mathbb{C}^N, \|\cdot\|_2)$ --- for instance by bounding
$\|DF\|_{\mathrm{op}} < 1$ uniformly --- remains open. The numerical
evidence (all tested cases converge to the same fixed point regardless of
initial condition) strongly supports uniqueness, but a proof is not yet
available.
\end{proof}
...
carries $c$-dependent quantum information. No separate total Hilbert
space $\mathcal{H}_C \otimes \mathcal{H}_S$ was introduced as an axiom;
the inner product structure on $\mathbb{C}^{d_S}$ that makes
$\rho_{S|c}$ interpretable as a quantum density matrix is inherited from
the $L^2$ normalization of $\psi^*$, not independently derived.
\end{remark}

This is how Axiom~\ref{ax:nohilbert} is realized: Hilbert space is not
introduced as a separate axiom but emerges as the natural structure on
the conditioned subsystem, inherited from the self-consistency measure.
Whether this inheritance constitutes a full derivation of Hilbert
structure from non-Hilbert inputs, or a relocation of that structure
from axiom to normalization convention, remains an open question
...
\label{eq:sce}
\end{equation}
The companion paper~\cite{fixed_point} argues that eq.~\eqref{eq:sce}
has a self-consistent solution (Sketch for massive fields with
$m \ll M_P$; demoted from Rigorous in June 2026). Without mass, the only stress-energy source is the conformal
...
\midrule
Lorentzian & $\checkmark$ (Sketch) & $\checkmark$ (Sketch) & $\checkmark$ (Rigorous) & $\times$ \\
...
The Lorentzian massive Level~2 entry is Sketch (Banach contraction,
$\kappa \sim (m/M_P)^2$, companion paper~\cite{fixed_point}; demoted
from Rigorous in June 2026 after review identified gaps in the kernel
bound and contraction estimate); the
Riemannian massive entry is likewise Sketch (same contraction argument applies to
...
fields (Sketch; demoted from Rigorous in June 2026, and \emph{permanently}
Sketch at this generality after the governing gap was found to be a
structural obstruction: the non-local response kernel is a Green operator of
the Lorentzian hyperbolic operator, with causal-past support, so the
single-Cauchy-surface bound used in the companion paper cannot be closed
without a controlled global foliation beyond its assumptions; any repair is
then foliation-restricted (companion paper~\cite{fixed_point}, Section~3 and
\texttt{explorations/2026-06-16-FPE4-M3-structural-obstruction.md}). The
extension to massless fields is blocked by the \emph{same} structural gap, not
by an independent infrared failure: on a compact Cauchy surface the spectral
gap stays positive, but it controls the spatial elliptic operator, which is
not the Lorentzian object at issue.
...
surfaces, under six stated assumptions of which five are established and
one (a uniform stress-energy bound) remains conjectural (Conditional).

\end{enumerate}

An earlier claim that the Planck scale emerges as the natural validity
boundary is withdrawn: the supporting estimate contradicted the companion
paper's own $\kappa$ formula, and a corrected estimate is open work
(companion paper~\cite{fixed_point}, Section~7).
...
Lorentzian and Riemannian metrics. The Banach contraction argument
supports existence of self-consistent solutions regardless of signature
(at Sketch level, matching that argument's current status). For
...
\emph{Caveat (proof strategy, not existence).} The signature-blindness of
\emph{existence} rests on the two exact fixed points just cited and is not in
question. The signature-blindness of the \emph{Banach-contraction argument},
however, is exactly where that argument's structural gap (M3) lives: the kernel
bound is governed by $\Box_g$, which is \emph{hyperbolic} on a Lorentzian metric
(causal-past support --- the obstruction) but \emph{elliptic} on a Riemannian
metric (where a spatial-resolvent bound is legitimate). The ``identical $\kappa$
for both signatures'' is therefore, at the level of the proof, the very
Lorentzian-vs-Riemannian conflation the companion paper flags; whether this
sharpens or weakens the signature-blindness reading is an open question recorded
for adjudication, not resolved here.
\end{remark}
...
The Banach contraction result (Sketch; permanently at this generality due to
the structural M3 obstruction --- see Section~\ref{sec:level2}) is the
perturbative stability of the fixed point at this
level. The Schauder existence result (conditional on one open assumption,
a uniform stress-energy bound)
...
$\psi^*$ retains $\sim 25$--$40\%$ imaginary norm (after global phase
alignment; the value is set by $\theta$, the spread of $h$, $N$, and the
alignment convention via the exact reduction
eq.~\eqref{eq:im_frac_reduction}, and is independent of subsystem rank)
at all sizes tested through $N = 256$, while the Riemannian $\psi^*$ is
exactly real. The
...
However, in the semiclassical clock limit
($d_{\mathrm{clock}} \gg d_{\mathrm{sys}}$ with structured interaction
$h_{(c,s)} = h^S_s + (c/d_{\mathrm{clock}}) V_s$), unitarity
\emph{emerges}: the orbit error converges to zero, the conditional
states lie on a one-parameter unitary orbit, and the effective
Hamiltonian $H_{\mathrm{eff}} \approx \theta V / d_{\mathrm{clock}}$
is self-consistently determined \emph{(Sketch)}.
Crucially, this requires Lorentzian signature ($\theta > 0$):
the phase-lock lemma guarantees that magnitude profiles are
$\theta$-independent, so all $c$-dependent phase structure---and
hence all unitary evolution---originates from $\theta \neq 0$.
At $\theta = 1$ with $d_{\mathrm{clock}} = 256$ and
$V_{\mathrm{scale}} \cdot d_{\mathrm{clock}} = 16$, the
Hamiltonian reconstruction fidelity reaches $0.9996$.
The measure-theoretic construction thus resolves both the
\emph{algebraic} circularity (no total Hilbert space needed) and,
in the semiclassical limit, the \emph{dynamical} content of
Page--Wootters (unitary time evolution with self-consistently
determined Hamiltonian). A rigorous convergence proof remains open.
...
$\psi^*$ retains $\sim 25$--$40\%$ imaginary norm (after global phase
alignment; set by $\theta$, the spread of $h$, $N$, and the alignment
convention via the exact reduction eq.~\eqref{eq:im_frac_reduction},
independent of subsystem rank) at all sizes tested through
