...
$\mathrm{Tr}(\rho^2)$ (using the Hermiticity of $\rho$). Part~(b), the
rank-2 von Neumann excess, is proved as \texttt{rank2\_entropy\_excess}
by reducing it to the monotonicity of the binary entropy on
$[\tfrac12,1]$; there Fact~2 (the Perron--Frobenius decrease
$\sigma_1(\theta)\le\sigma_1$, proved analytically above) enters as an
\emph{explicit hypothesis} rather than being re-derived in Lean---the
single dependency the formalization imports rather than checks. An
\texttt{\#print axioms} audit shows the theorems rest only on Mathlib's
standard axioms, with no \texttt{sorry}. Consistent with
