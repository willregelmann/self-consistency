...
radius $R$, sound speed $c_s$. In the well-separated regime $d\gg R$ the cross term in
$E_\Delta$ (eq.~\eqref{eq:tau_dp}) is negligible and the gravitational coherence energy
reduces to twice the Newtonian self-energy of a uniform sphere,
\begin{equation}
E_\Delta \;\xrightarrow{\,d\gg R\,}\; \frac{6}{5}\,\frac{Gm^2}{R},
\label{eq:Edelta_selfenergy}
\end{equation}
which reproduces Table~\ref{tab:platforms} (e.g.\ $\tau_{DP}=\hbar/E_\Delta=11.6$~ms for
the BMV microdiamond). The lowest acoustic mode scales as $\omega_{\text{phonon}}\sim
c_s/L$ with the body's linear size $L\simeq 2R$. Both frequencies scale as $R^{-1}$
($\omega_{\text{phonon}}\propto c_s/R$, $\omega_g=E_\Delta/\hbar\propto Gm^2/(\hbar R)$),
so the radius cancels and the ratio is fixed by the material sound speed and total mass
alone:
\begin{equation}
\frac{\omega_{\text{phonon}}}{\omega_g}
= \frac{c_s\hbar}{L\,E_\Delta}
= \frac{5\,\hbar c_s}{12\,G m^2}.
\label{eq:crossover_ratio}
\end{equation}
The crossover $\omega_{\text{phonon}}=\omega_g$ therefore occurs at a single critical
mass, independent of body size,
\begin{equation}
m_\ast = \sqrt{\frac{5\,\hbar c_s}{12\,G}},
\label{eq:mstar}
\end{equation}
or, at fixed density $\rho$, a critical radius $R_\ast=(3m_\ast/4\pi\rho)^{1/3}$. This
threshold is \textbf{(Sketch)}: the arithmetic and the $R$-cancellation are exact, but the
...
\begin{tabular}{lccc}
\toprule
Material & $c_s$ (m/s) & $m_\ast$ (kg) & $R_\ast$ ($\mu$m) \\
\midrule
Diamond (rigid) & $1.8\times10^{4}$ & $1.1\times10^{-10}$ & $20$ \\
Fused silica    & $5.9\times10^{3}$ & $6\times10^{-11}$   & $19$ \\
Aerogel (soft)  & $1\times10^{2}$   & $8\times10^{-12}$   & $27$ \\
\bottomrule
\end{tabular}
\end{center}
Even the softest realistic solid places the crossover at $m_\ast\gtrsim10^{-11}$~kg and
$R_\ast\sim20$--$27\,\mu$m---three to four orders of magnitude heavier, and an order of
magnitude larger, than the most massive \emph{proposed} superposition platform (BMV,
$m\sim10^{-14}$~kg, $R\sim1\,\mu$m; Table~\ref{tab:platforms}). Maintaining a spatial
superposition of a $\sim\!20\,\mu$m body over the separation $d\gtrsim R$ that the
gravitational coherence energy~\eqref{eq:Edelta_selfenergy} requires lies far outside the
coherence window of any current or near-future experiment, where environmental
