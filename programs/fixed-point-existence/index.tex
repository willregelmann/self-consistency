...
Cauchy surfaces, under five stated assumptions of which one remains an open conjecture,
plus an explicit semiclassical hypothesis on $\hbar$ (Section~\ref{sec:schauder}).
...
(Section~\ref{sec:schauder}) remains conditional on an open conjecture~(A3) and
an explicit semiclassical hypothesis on~$\hbar$. An earlier claim that the Planck scale
...
surface $\Sigma$. Fix $s > n/2 + 2 = 4$ and define the candidate fixed-point set:
\begin{equation}
K_\rho \equiv \bigl\{h \in H^s(\Sigma) : \|h-\bar{g}\|_{H^s} \leq \rho\bigr\}
\cap \mathcal{G}_{\text{harm}} \cap \mathcal{L}
\label{eq:K_rho}
\end{equation}
where $\mathcal{G}_{\text{harm}}$ is the closed affine subspace of metrics in linearized
harmonic gauge and $\mathcal{L}$ is the open set of Lorentzian-signature metrics. The
required convexity and compactness of $K_\rho$ are established in the proof of
Theorem~\ref{thm:schauder} below: for $\rho$ below a threshold set by the Sobolev
embedding constant, $\overline{B}_\rho(\bar{g}) \subset \mathcal{L}$ and the intersection
with $\mathcal{L}$ is vacuous, reducing $K_\rho$ to the intersection of a closed convex
ball with a closed affine subspace — hence closed and convex in $H^s$, compact in
$H^{s'}$ for $s' < s$ by Rellich--Kondrachov.
...
\emph{(A1)--(A5)} and the semiclassical hypothesis
$\hbar < \hbar_{\max}$~\eqref{eq:hbar_max}, the self-consistency map $\mathcal{F}$ has at
least one fixed point $g^* \in K_{\rho^*}$ in the $H^{s'}$ topology for any $s' < s$,
where $\rho^* = C_{\text{inv}}B_0\hbar$~\eqref{eq:rho_star}. Assumption~(A3) enters
through the uniform bound used in Step~2.
\end{theorem}

\noindent\emph{Proof.} \textbf{(Rigorous, conditional on (A3).)} We verify the hypotheses
of the Schauder fixed-point theorem~\cite{schauder}.

\smallskip\noindent\textit{Step 1: $K_\rho$ is convex, closed, and compact (for $\rho$
below a threshold).}
Since $s > n/2 + 2 = 4$ and $\Sigma$ is compact, the Sobolev embedding
$H^s(\Sigma) \hookrightarrow C^2(\Sigma)$ holds with embedding constant $C_{\mathrm{emb}}$:
$\|h\|_{C^2} \leq C_{\mathrm{emb}}\|h\|_{H^s}$ for all $h \in H^s(\Sigma)$.
The background $\bar{g}$ is Lorentzian, so its eigenvalues $\lambda_i(\bar{g}(x))$ satisfy
$\lambda_{\min} \equiv \min_{x \in \Sigma}\min_i |\lambda_i(\bar{g}(x))| > 0$.
For any $g$ with $\|g - \bar{g}\|_{H^s} \leq \rho$, the $C^0$-distance satisfies
$\|g - \bar{g}\|_{C^0} \leq C_{\mathrm{emb}}\rho$, and eigenvalue perturbation gives
$|\lambda_i(g(x)) - \lambda_i(\bar{g}(x))| \leq C_{\mathrm{emb}}\rho$ pointwise.
Choose the Lorentzian-signature threshold:
\begin{equation}
\rho_{\mathcal{L}} \equiv \frac{\lambda_{\min}}{C_{\mathrm{emb}}}.
\label{eq:rho_L}
\end{equation}
For $\rho < \rho_{\mathcal{L}}$, every $g$ in the closed $H^s$ ball
$\overline{B}_\rho(\bar{g}) \equiv \{h \in H^s(\Sigma) : \|h - \bar{g}\|_{H^s} \leq \rho\}$
has signature identical to $\bar{g}$ at every point of $\Sigma$; in particular it is
Lorentzian. Thus the intersection with~$\mathcal{L}$ is vacuous
($\overline{B}_\rho(\bar{g}) \subset \mathcal{L}$), and
\[
K_\rho = \overline{B}_\rho(\bar{g}) \cap \mathcal{G}_{\text{harm}}
\quad \text{for } \rho < \rho_{\mathcal{L}}.
\]
The closed ball $\overline{B}_\rho(\bar{g})$ is closed and convex in $H^s$;
$\mathcal{G}_{\text{harm}}$ is a closed affine subspace (the linearized harmonic-gauge
condition $\nabla^\mu(h_{\mu\nu} - \tfrac{1}{2}\bar{g}_{\mu\nu}\mathrm{tr}\,h) = 0$ is
a closed linear constraint). Hence $K_\rho$ is closed and convex in $H^s$.
By the Rellich--Kondrachov theorem on compact~$\Sigma$, the embedding
$H^s(\Sigma) \hookrightarrow H^{s'}(\Sigma)$ is compact for $s' < s$, so the closed
bounded set $K_\rho$ is compact in the $H^{s'}$ topology.

\smallskip\noindent\textit{Step 2: $\mathcal{F}$ maps $K_\rho$ into $K_\rho$
(semiclassical $\hbar$-smallness regime).}
Given $g \in K_\rho$: (A1) provides a Hadamard state with well-defined renormalized
stress-energy; (A5) reduces the fourth-order system to second order; (A4) yields
$\mathcal{F}(g) \in H^s$ with
$\|\mathcal{F}(g) - \bar{g}\|_{H^s} \leq C_{\text{inv}}\|S_{\mu\nu}[g]\|_{H^{s-2}}
\leq C_{\text{inv}}B(\rho)$
by~(A3).

\emph{$\rho$-selection.} The renormalized stress-energy carries an overall factor of
$\hbar$ from the one-loop structure of $\langle\hat{T}_{\mu\nu}\rangle_{\text{ren}}$:
\begin{equation}
B(\rho) \leq B_0\,\hbar \quad \text{for a constant } B_0 \text{ depending on }
(\bar{g}, m, \xi, s) \text{ but not on } \rho,
\label{eq:B_hbar}
\end{equation}
where the $\rho$-independence holds because the leading-order stress-energy is
determined by the background geometry (the state-dependence on $g$ contributes at
$O(\hbar\rho)$ — a subleading correction that is absorbed into$~B_0$ for
$\rho \leq 1$). The self-mapping condition $C_{\text{inv}}B(\rho) \leq \rho$ is therefore
satisfied by choosing
\begin{equation}
\rho^* = C_{\text{inv}}B_0\,\hbar.
\label{eq:rho_star}
\end{equation}
In the semiclassical regime $\hbar \ll \lambda_{\min}/(C_{\mathrm{emb}}C_{\text{inv}}B_0)$,
this gives $\rho^* < \rho_{\mathcal{L}}$, so Step~1 applies.
The condition on $\hbar$ is stated explicitly:
\begin{equation}
\hbar < \hbar_{\max} \equiv \frac{\lambda_{\min}}{C_{\mathrm{emb}}\,C_{\text{inv}}\,B_0}.
\label{eq:hbar_max}
\end{equation}
This is the \emph{semiclassical hypothesis}: the quantum stress-energy must be small
enough that the geometry is not driven away from the Lorentzian signature of the
background.

\emph{Harmonic-gauge preservation.} The output $\mathcal{F}(g)$ lies in
$\mathcal{G}_{\text{harm}}$ by construction: (A4) solves the linearized Einstein equation
\emph{in} harmonic gauge — the gauge condition is built into the solution operator, not
an ex post constraint on the output. Explicitly, (A4) inverts the reduced (gauge-fixed)
Einstein operator, whose solution automatically satisfies the linearized de~Donder
condition $\nabla^\mu(\mathcal{F}(g)_{\mu\nu} -
\tfrac{1}{2}\bar{g}_{\mu\nu}\mathrm{tr}\,\mathcal{F}(g)) = 0$; the contracted Bianchi
identity $\nabla^\mu G_{\mu\nu} = 0$ together with $\nabla^\mu S_{\mu\nu} = 0$ (the
latter following from the Ward identity for $\langle\hat{T}_{\mu\nu}\rangle_{\text{ren}}$
evaluated at the solution~$g$) ensures the gauge condition propagates. Therefore
$\mathcal{F}(g) \in \mathcal{G}_{\text{harm}}$, and combined with the norm bound above,
$\mathcal{F}(g) \in K_{\rho^*}$.
